...
class Acharset
{
    ...
    static protected $HTTP_ACCEPT_LANGUAGE = array();

    public static function _charset_count_bad($s)
    ...
    public static function _charset_count_chars($s)
    ...
    public static function _charset_count_pairs($s)
    ...
    public static function _charset_alt_win($s)
    {
        for ($i = 0, $iMax = strlen($s); $i < $iMax; $i++) {
            $c     = ord($s[$i]);
            if ($c >= 0x80 && $c <= 0x9f)
    ...
    public static function _charset_koi_win($s)
    {
        $kw = array(
    ...
    public static function _charset_utf8_win($s)
    {
        $r     = '';
        $state = 1;
        for ($i = 0, $iMax = strlen($s); $i < $iMax; $i++) {
    ...
    public static function _charset_prepare($s)
    {
        $r = 0;
        $k = 0;
...
// returns a string converted from a best encoding (windows-1251 or koi-8r) to windows-1251
    ...
    static protected function _serverAcceptLang()
    {
        $l  = explode(',', filter_input(INPUT_SERVER, 'HTTP_ACCEPT_LANGUAGE'));
        $ll = array();
        for ($i = 0; $i < count($l); $i++) {
            $t = explode(';', $l[$i]);
            if (count($t) > 1) {
        ...
        self::$HTTP_ACCEPT_LANGUAGE = $ll;
        return $ll;
    }

    public static function serverDefaultLang()
    {
        if (!self::$HTTP_ACCEPT_LANGUAGE) {
            self::_serverAcceptLang();
        }
        return self::$HTTP_ACCEPT_LANGUAGE['language'];
    }
